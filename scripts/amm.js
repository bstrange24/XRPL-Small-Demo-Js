...
// rJQBUqSuRrshJDLbj2FbXjZX8niz74Wrhv
// sEd7qcZGR6LUN5b77XfEfXPwMvVaSKD
// r9J3Lh17mw52nAA84ajswtf4fHL7MKGm45
// sEdV9aY6bSYoWh1g3JnkQU8gbbdJaNm
// rEptGARA1CsLZW6s9dS2Fuqyoq1mG81Xgv
// sEd7FWJ2jeCyWGjx6XrVvhZAbUvGA7c

const SEED_WARM = 'sEd7qcZGR6LUN5b77XfEfXPwMvVaSKD'; // performs swap
const SEED_HOT = 'sEdV9aY6bSYoWh1g3JnkQU8gbbdJaNm'; // deposits liquidity
const SEED_COLD_ISSUER = 'sEd7FWJ2jeCyWGjx6XrVvhZAbUvGA7c'; // issues BOB
...
export async function ammDeposit(client, wallet, assetDef, asset2Def, options) {
     ...
     if (options.amount && options.amount2) {
          ...
          // Single-sided XRP (Asset)
          tx.Amount = options.amount; // String of drops
          tx.Flags = xrpl.AMMDepositFlags.tfSingleAsset;
     } else if (options.singleSide === 'iou') {
          // Single-sided IOU (Asset2)
          tx.Amount = options.amount;
          tx.Flags = xrpl.AMMDepositFlags.tfSingleAsset;
     } else {
...
export async function ammWithdraw(client, wallet, assetDef, asset2Def, options) {
     ...
     if (!options.oneSide) {
          // Double-sided withdrawal
          if (options.amount && options.amount2) {
               // Exact amounts of both assets (tfTwoAsset)
               tx.Amount = options.amount;
               tx.Amount2 = options.amount2;
               tx.Flags = xrpl.AMMWithdrawFlags.tfTwoAsset;
          } else if (options.lpTokenIn) {
               // Proportional both assets from LP burn (tfLPToken)
               tx.LPTokenIn = options.lpTokenIn;
               tx.Flags = xrpl.AMMWithdrawFlags.tfLPToken;
          } else {
               // All LP for both assets (tfWithdrawAll)
               tx.Flags = xrpl.AMMWithdrawFlags.tfWithdrawAll;
          }
     } else {
          // Single-sided withdrawal
          // Set Amount for the target asset (XRP or IOU)
          if (options.amount) {
               // Withdraw XRP (Asset)
               tx.Amount = options.amount; // String of drops
          } else if (options.amount2) {
               // Withdraw IOU (Asset2)
               tx.Amount = options.amount2; // IOU object
          } else {
               throw new Error('For oneSide, provide amount (XRP) or amount2 (IOU)');
          }

          // Set mode based on whether LPTokenIn is provided
          if (options.lpTokenIn) {
               // Max out with max LP burn (tfOneAssetLPToken)
               tx.LPTokenIn = options.lpTokenIn;
               tx.Flags = xrpl.AMMWithdrawFlags.tfOneAssetLPToken;
          } else {
               // Exact out with auto LP burn (tfSingleAsset)
               tx.Flags = xrpl.AMMWithdrawFlags.tfSingleAsset;
          }
     }
...
(async () => {
     ...
     const { lpTokenDef, lpBalance } = await getLpTokenAndBalance(client, hot.classicAddress, ASSET_XRP, ASSET_USD);
     ...
     // console.log('\n----------------- ammDeposit Both Sides -----------------');
     // await ammDeposit(client, warm, ASSET_XRP, ASSET_USD, {
     //      amount: '5000000', // 5 XRP in drops
     //      amount2: { currency: 'BOB', issuer: issuer.classicAddress, value: '10' },
     // });
     ...
     // await ammDeposit(client, warm, ASSET_XRP, ASSET_USD, {
     //      singleSide: 'iou',
     //      amount: { currency: 'BOB', issuer: issuer.classicAddress, value: '10' },
     // });

     // // console.log('\n----------------- ammWithdraw one-sided BOB (max 5 BOB with max LP burn) -----------------');
     // // await ammWithdraw(client, warm, ASSET_XRP, ASSET_USD, {
     // //      lpTokenIn: lpTokenDef, // Optional: caps LP burn; omit for exact
     // //      oneSide: true,
     // //      amount2: { currency: 'BOB', issuer: issuer.classicAddress, value: '5' },
     // // });

     // console.log('\n----------------- ammWithdraw Exact 5 BOB (auto-burn LP, no cap) -----------------');
     // await ammWithdraw(client, warm, ASSET_XRP, ASSET_USD, {
     //      oneSide: true,
     //      amount2: { currency: 'BOB', issuer: issuer.classicAddress, value: '5' },
     // });

     // console.log('\n----------------- ammWithdraw Proportional both (burn specific LP) -----------------');
     // await ammWithdraw(client, warm, ASSET_XRP, ASSET_USD, {
     //      lpTokenIn: lpTokenDef,
     // });

     // console.log('\n----------------- ammWithdraw Exact both (up to amounts) -----------------');
     // await ammWithdraw(client, warm, ASSET_XRP, ASSET_USD, {
     //      amount: '1000000', // 1 XRP drops
     //      amount2: { currency: 'BOB', issuer: issuer.classicAddress, value: '10' },
     // });

     // console.log('\n----------------- ammWithdraw Full both (all LP) -----------------');
     // await ammWithdraw(client, warm, ASSET_XRP, ASSET_USD, {});
